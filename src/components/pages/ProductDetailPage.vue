...
export default {
  ...
  computed: {
    otherProducts() {
      ...
      return this.allProducts.filter((p) => p.id !== currentId).slice(0, 4);
    },
  },
  watch: {
    "$route.params.id": {
  ...
  methods: {
    ...
    async checkWishlistStatus(productId) {
      try {
        const wishlistRef = child(
          dbRef(rtdb),
          `wishlists/${this.currentUser.uid}`
        );
        const snapshot = await get(wishlistRef);
        if (snapshot.exists()) {
          const data = snapshot.val();
          const itemKey = Object.keys(data).find(
            (key) => data[key].productId === productId
          );
    ...
    // Di dalam methods ProductDetailPage.vue
    async handleBuyNow() {
      if (!this.currentUser) {
      ...
      // Tandai bahwa ini adalah pembelian langsung (Direct Buy)
      const directBuyItem = [
        {
          productId: this.product.id,
          name: this.product.name,
      ...
      // Simpan data ke localStorage khusus untuk sesi checkout ini
      localStorage.setItem("vans_checkout_mode", "direct");
      localStorage.setItem("vans_direct_item", JSON.stringify(directBuyItem));
